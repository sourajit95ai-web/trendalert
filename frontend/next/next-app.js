...
(function () {
  ...
  function railHTML(all) {
    ...
    /* BIG MOVERS -- took the slot "Recent alerts" used to hold, because a name
       that moved hard TODAY is the thing worth seeing first. Crosses were not
       deleted, only demoted: they now sit below sectors, which is the right
       order of urgency for a rail read top to bottom.

       Both blocks share the .alert-row markup and the data-act="open" wiring,
       so every row in either one is a click from its chart.

       The empty state stays. This block owns a permanent slot, and a heading
       that vanishes on a quiet day reads as a broken section rather than a
       calm market. */
    ...
    /* data.json carries only crosses detected on the LATEST bar and keeps no
       history, so there is nothing to age these by -- every row is today's, and
       the label says so rather than stamping each row with a date it cannot
       support. An alert history file is a backend change, noted in OPS.md. */
    const al = (s.payload && s.payload.alerts) || [];
    const crosses = `
      <div>
        <div class="rail-label">Trend crosses · latest bar</div>
        ${al.length ? al.map(a => `
          ...
          : `<p class="rail-note">No EMA ${esc(s.settings.trendFast)} / ${esc(s.settings.trendSlow)} crosses on the latest bar.</p>`}
      </div>`;
    ...
    return summary + alerts + sectors + crosses + note;
  }
